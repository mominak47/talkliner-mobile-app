...
class FcmService extends GetxService {
  ...
  Future<FcmService> onInitService() async {
    await _initialize();
    return this;
  }
  ...
  Future<void> _initialize() async {
    ...
    if (settings.authorizationStatus == AuthorizationStatus.authorized) {
      ...
      if (Platform.isIOS) {
        ...
        if (apnsToken == null) {
          debugPrint('[FCM] APNS token is null. Waiting for token...');
          for (int i = 0; i < 3; i++) {
            await Future.delayed(const Duration(seconds: 3));
            apnsToken = await _firebaseMessaging.getAPNSToken();
            if (apnsToken != null) {
              debugPrint('[FCM] APNS token received after retry ${i + 1}');
              break;
            }
          }
        }
        ...
        if (apnsToken != null) {
          ...
          debugPrint(
            '[FCM] Failed to get APNS token after retries. FCM token usually fails without it on iOS.',
          );
        }
      }

      // Get FCM Token
      // Only attempt to get FCM token if we are not on iOS OR if we are on iOS and have an APNS token
      // actually getAPNSToken is a prerequisite for getToken on iOS usually, but let's try anyway or guard it.
      // The error "[firebase_messaging/apns-token-not-set]" suggests strict dependency.
